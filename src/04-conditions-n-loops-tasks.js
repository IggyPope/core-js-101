...
function isTriangle(a, b, c) {
  ...
  return [a, b, c].reduce((result, side) => {
    if (side >= (sum - side)) return false;
    return result;
  }, true);
}
...
function isCreditCardNumber(ccn) {
  const digits = ccn.toString().split('').map((item) => +item);

  const checkSum = digits.reduceRight((cSum, digit, i, arr) => {
    if ((arr.length - 1 - i) % 2) {
      return cSum + digit * 2 - ((digit > 4) && 9);
    }
    return cSum + digit;
  }, 0);
...
function getDigitalRoot(num) {
  const numArray = num.toString().split('').map((item) => +item);

  const digitsSum = numArray.reduce((acc, curr) => acc + curr, 0);

  if (digitsSum.toString().length > 1) return getDigitalRoot(digitsSum);
    ...
    input.forEach((bracket) => {
      if (openingBrackets.includes(bracket)) {
        ...
        if (!lastBracket || bracketsMap.get(lastBracket) !== bracket) {
          throw new Error('Bracket mismatch or no opening bracket found.');
        }
      }
    });
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i <= pathes[0].length; i += 1) {
    ...
    const isCommon = pathes.slice(1).reduce((acc, curr) => {
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    /* check if one of the rows is the winning one */
    if (position[i][0] && position[i][0] === position[i][1] && position[i][1] === position[i][2]) {
      return position[i][0];
    }
    /* check if one of the columns is the winning one */
    if (position[0][i] && position[0][i] === position[1][i] && position[1][i] === position[2][i]) {
      return position[0][i];
    }
  }
  /* check if one of the diagonals is the winning one */
